        ...
        <div className="p-6 space-y-4">
          ...
          {coupon ? (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
              ...
              <div className="font-mono font-bold text-2xl px-4 py-3 rounded-xl tracking-[0.3em]"
      ...
      <div className="p-5 flex-1">
        <div className="flex items-start justify-between mb-3">
          ...
          <div className="flex flex-col items-end gap-1">
            ...
            {item.tierRequired && item.tierRequired !== 'SILVER' && (
              <span className="text-xs px-2 py-0.5 rounded-full font-medium" style={{ background: tierS.bg, color: tierS.text }}>
                <TierIcon fontSize="inherit" /> {item.tierRequired}+
              </span>
            )}
          </div>
        </div>
          ...
          {summary?.nextTier && (
            <>
              <div className="w-full h-2 rounded-full overflow-hidden mt-2" style={{ background: isDark ? '#2d4263' : 'var(--border)' }}>
                <motion.div className="h-full rounded-full" style={{ background: 'linear-gradient(90deg,var(--brand),#4ade80)' }}
                  initial={{ width: 0 }}
                  animate={{ width: `${Math.max(8, 100 - ((summary.pointsToNextTier ?? 0) / ((summary.pointsToNextTier ?? 0) + pts) * 100))}%` }}
