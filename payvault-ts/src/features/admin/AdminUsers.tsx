...
export function AdminUsers() {
  ...
  const handleBlock = async (u: AdminUserResponse, block: boolean) => {
    ...
    setActioning(true)
    try {
      if (block) await adminService.blockUser(u.id, role)
      else await adminService.unblockUser(u.id, role)
  ...
  const handleRole = async () => {
    ...
    if (!roleResult.success) {
      toast.error(getFirstError(roleResult.error))
      return
    }
    setActioning(true)
    try {
      const nextRole = roleResult.data as UserRole
      await adminService.changeRole(selected.id, nextRole, role)
      const changedOwnRole = !!user && selected.id === user.id && user.role !== nextRole
      if (changedOwnRole && user) {
        if (refreshToken) {
          try {
            ...
            dispatch(updateCurrentUser({ ...user, role: nextRole }))
          }
        } else {
          dispatch(updateCurrentUser({ ...user, role: nextRole }))
        }
      ...
      setSelected(null)
      setRoleTo('')
      if (changedOwnRole && nextRole !== 'ADMIN') {
  ...
  return (
    <div className="p-4 lg:p-6 space-y-5 max-w-5xl mx-auto">
              ...
              >
                ...
                <div className="p-5 space-y-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="font-semibold" style={{ color: 'var(--text-primary)' }}>{selected.name}</div>
                  ...
                  <div className="flex gap-3">
                    {selected.status === 'ACTIVE'
                      ? (
                        <button
                          onClick={() => handleBlock(selected, true)}
                          disabled={actioning || selected.id === user?.id}
                          className="flex-1 btn-danger py-2.5 text-sm inline-flex items-center justify-center gap-1"
                          style={{ opacity: actioning || selected.id === user?.id ? 0.6 : 1 }}
                        >
                          <Icon8 name="blocked" size={14} />
                          {selected.id === user?.id ? 'Cannot Block Self' : 'Block'}
                        </button>
                      )
                      : (
                        <button
                          onClick={() => handleBlock(selected, false)}
                          disabled={actioning}
                          className="flex-1 py-2.5 rounded-xl text-sm font-semibold inline-flex items-center justify-center gap-1"
                          style={{ background: '#dcfce7', color: '#16a34a' }}
                        >
                          <Icon8 name="success" size={14} />
                          Unblock
                        </button>
                      )}
      ...
      <div>
        <h1 className="text-xl font-display font-bold" style={{ color: 'var(--text-primary)' }}>User Management</h1>
      </div>
      ...
      <motion.div className="card overflow-hidden" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
        ...
        {loading
          ...
          : users.length === 0
            ? <div className="text-center py-12"><div className="inline-flex mb-3"><Icon8 name="transactions" size={36} /></div><p style={{ color: 'var(--text-muted)' }}>No users found</p></div>
            : (
              <>
