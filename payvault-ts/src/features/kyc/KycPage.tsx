...
export function KycPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!docNumber.trim()) { toast.error('Enter your document number'); return }
    if (!file) { toast.error('Upload your document'); return }
    if (!user?.id) return
    setSubmitting(true)
    try {
      const { data } = await kycService.submit(user.id, docType, docNumber, file)
      setKycData(data.data); dispatch(updateKycStatus('PENDING'))
  ...
  // Allow initial submission when backend indicates "NOT_SUBMITTED",
  // and allow resubmission when "REJECTED".
  ...
  const kycDetailRows: [string, string][] = kycData
    ? [
        ...
        ...(kycData.updatedAt
          ? [['Updated', formatDate(kycData.updatedAt, 'DD MMM YYYY')]] as [string, string][]
          : []),
      ]
    : []
  ...
  return (
    <div className="p-4 lg:p-6 max-w-2xl mx-auto space-y-5">
      <div><h1 className="text-xl font-display font-bold" style={{ color: 'var(--text-primary)' }}>KYC Verification</h1>
        <p className="text-sm mt-0.5" style={{ color: isDark ? '#9fb4d7' : 'var(--text-secondary)' }}>Verify your identity to unlock all wallet features</p></div>
        ...
        <motion.div
          ...
          <div className="flex items-start gap-4">
            <div
              ...
              style={{ background: kycI.bg, color: kycI.color }}
            >
      ...
      {canSubmit && (
        <motion.div className="card p-5" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
          ...
          <p className="text-xs mb-5" style={{ color: 'var(--text-muted)' }}>Accepted: Aadhaar, PAN, Passport, Driving License · Max 5MB</p>
          <form onSubmit={handleSubmit} className="space-y-4" noValidate>
            <div>
              <label htmlFor="docType" className="block text-xs font-semibold mb-1.5 uppercase tracking-wider" style={{ color: 'var(--text-secondary)' }}>Document Type</label>
            ...
            <div>
              <label htmlFor="docNumber" className="block text-xs font-semibold mb-1.5 uppercase tracking-wider" style={{ color: 'var(--text-secondary)' }}>Document Number</label>
              <input id="docNumber" type="text" placeholder="e.g. ABCDE1234F" value={docNumber}
                onChange={e => setDocNumber(e.target.value)} className="input-field font-mono" required />
            </div>
            <div>
              ...
              <div
                ...
                style={{ borderColor: drag || file ? 'var(--brand)' : 'var(--border)', background: drag || file ? 'var(--brand-light)' : 'var(--bg-primary)' }}
                onClick={() => fileRef.current?.click()}
                onDragOver={e => { e.preventDefault(); setDrag(true) }}
                onDragLeave={() => setDrag(false)}
                onDrop={e => { e.preventDefault(); setDrag(false); if (e.dataTransfer.files[0]) handleFile(e.dataTransfer.files[0]) }}
                role="button" tabIndex={0} aria-label="Upload document file"
                onKeyDown={e => e.key === 'Enter' && fileRef.current?.click()}>
                <input ref={fileRef} type="file" className="hidden" accept="image/*,.pdf"
                  onChange={e => e.target.files?.[0] && handleFile(e.target.files[0])} aria-hidden="true" />
                {file ? (
                  <><div className="inline-flex mb-1"><Icon8 name="kyc" size={30} /></div>
                    ...
                    <div className="text-xs mt-1" style={{ color: 'var(--text-muted)' }}>{(file.size / 1024).toFixed(0)} KB · Click to replace</div></>
                ) : (
                  <><div className="inline-flex mb-1"><Icon8 name="overview" size={30} /></div>
                    <div className="text-sm font-medium" style={{ color: 'var(--text-secondary)' }}>Drag & drop or click to upload</div>
                    <div className="text-xs mt-1" style={{ color: 'var(--text-muted)' }}>PNG, JPG, PDF · max 5MB</div></>
                )}
              </div>
            </div>
