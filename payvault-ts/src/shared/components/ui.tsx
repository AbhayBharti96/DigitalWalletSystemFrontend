...
export const ConfirmDialog: React.FC<ConfirmProps> = ({
  open, onClose, onConfirm, title, message, amount, confirmLabel = 'Confirm', loading, danger,
}) => {
  return (
    <AnimatePresence>
      {open && (
...
export const StatusBadge: React.FC<{ status?: string | null }> = ({ status }) => {
  const map: Record<string, { bg: string; color: string }> = {
    SUCCESS: { bg: '#dcfce7', color: '#16a34a' }, COMPLETED: { bg: '#dcfce7', color: '#16a34a' },
  ...
  const c = map[safeStatus] || { bg: 'var(--bg-primary)', color: 'var(--text-muted)' }
  return (
