...
export const rewardsService = {
  summary: (userId: number) =>
    apiClient.get<ApiResponse<RewardSummary>>('/api/rewards/summary', { headers: { 'X-User-Id': userId } }),
  catalog: () => apiClient.get<ApiResponse<RewardItem[]>>('/api/rewards/catalog'),
  transactions: (userId: number) =>
    apiClient.get<ApiResponse<RewardTransaction[]>>('/api/rewards/transactions', { headers: { 'X-User-Id': userId } }),
  redeem: (userId: number, rewardId: number) =>
    apiClient.post<ApiResponse<Redemption>>('/api/rewards/redeem', { rewardId }, { headers: { 'X-User-Id': userId } }),
